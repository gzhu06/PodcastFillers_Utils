...
parser.add_argument('-dataset_path', required=True, type=str, help='folder path contains full-length MP3 podcast episodes')
...
def wav_cut(wavfile, start_time, end_time, output_file):
	...
	cut_cmd = ['ffmpeg', '-i', wavfile, '-ss', str(start_time), '-to', str(end_time), output_file]
...
def generate_clip_wav(master_csvfile, full_folder, clip_folder):
	...
	for i, event in event_df.iterrows():
		...
		src_filepath = os.path.join(full_folder, episode_subset, event['podcast_filename'] + '.wav')
